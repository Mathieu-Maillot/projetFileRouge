  ...
  persist(
    (set, get) => ({
      ...
      updateTraject: (trajectId, updateData) => {
        ...
        return updatedRides.find(ride => ride._id.$oid === trajectId);
      },
      ...
      bookTraject: (bookingData) => {
        ...
        const updatedRides = get().rides.map(ride => {
          if (ride._id.$oid === bookingData.rideId.$oid) {
            return {
              ...ride,
      ...
      cancelBooking: (bookingId) => {
        const bookingIndex = get().bookings.findIndex(
          booking => booking._id.$oid === bookingId
        );
        ...
        const updatedRides = get().rides.map(ride => {
          if (ride._id.$oid === booking.rideId.$oid) {
            return {
              ...ride,
      ...
      getUserTrajects: (userId) => {
        return get().rides.filter(ride => ride.driverId.$oid === userId);
      },
      
      getUserBookings: (userId) => {
        return get().bookings.filter(booking => booking.userId.$oid === userId);
      },
      
      getRideBookings: (rideId) => {
        return get().bookings.filter(booking => booking.rideId.$oid === rideId);
      },
